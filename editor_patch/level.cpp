...
static void compute_geoable_room_uids(CDedLevel& level, AlpineLevelProperties& props)
{
    ...
    xlog::debug("[Geoable] compute_room_uids: geoable_brush_uids={} rooms_count={}",
    ...
    for (std::size_t i = 0; i < props.geoable_brush_uids.size(); i++) {
        ...
        for (int j = 0; j < all_rooms.get_size(); j++) {
            ...
            if (brush_pos.x >= room->bbox_min.x - tolerance && brush_pos.x <= room->bbox_max.x + tolerance &&
                ...
                xlog::debug("[Geoable] matched brush uid={} pos=({:.1f},{:.1f},{:.1f}) -> room uid={}",
                            brush_uid, brush_pos.x, brush_pos.y, brush_pos.z, room->uid);
                break;
            }
        }

        if (props.geoable_room_uids[i] == 0) {
            xlog::warn("[Geoable] compute_room_uids: no room match for brush uid={} at ({:.1f},{:.1f},{:.1f})",
